...
const firstHalfFullLobby = (weightedLobbyPool) => {
  firstLobbyDict = weightedLobbyPool[0];
...
const isPlayerInAnotherLobby = (player) => {
...
export const createPlayer = new ValidatedMethod({
  ...
  run(player) {
    ...
    if (isPlayerInAnotherLobby(player)) {
      return;
    }
    ...
    const completedDemographics = didPlayerCompleteDemographics(player);
    let demographicsUpdate = { ['data.completedDemographics']: completedDemographics };
    Players.update(player._id, { $set: demographicsUpdate }, {
      autoConvert: false,
      filter: false,
      validate: false,
      trimStrings: false,
    ...
    console.log("Completed demographics: ", completedDemographics);
