...
export const createGameFromLobby = gameLobby => {
  ...
  players.forEach(player => {
    ...
    player.data["lobbyEndAt"] = new Date();
    const lobbyTime = (player.data["lobbyEndAt"] - player.data["lobbyStartAt"]) / 1000; // in seconds
    player.data["lobbyWaitTime"] = lobbyTime;
    player.data["bonus"] = lobbyTime / 60 * 0.16; // this is hardcoded here (meaning $0.16/min)
  });
