...
export const endOfStage = stageId => {
  ...
  let nextStage;
  if (round.data["nextRound"]) {
    nextStage = Stages.findOne({ gameId, index: index + game.data["numTurns"] - round.data["turnCount"] + 1 });
  } else {
    nextStage = Stages.findOne({ gameId, index: index + 1 });
  }
