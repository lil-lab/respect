...
const greedyNoMatches = (lobbies, playerId) => {
    myHash = playerId.split("_")[0];
    for (let i = 0; i < lobbies.length; i += 1) {
	// Get the earliest lobby possible
	currLobby = lobbies[i];

	// If the lobby is empty return
	if (currLobby.availableCount === 0) {
	    return currLobby;
	}

	// If lobby is not empty, check the player Ids
	playerRandId = currLobby.queuedPlayerIds[0];
	existing = Players.findOne({_id : playerRandId});
	if (!existing) {
	    return currLobby;
	} else {
	    playerHash = existing.id.split("_")[0];
	    if (!(playerHash === myHash)) {
		return currLobby;
	    }
	}
    }
...
export const createPlayer = new ValidatedMethod({
  ...
  run(player) {
    ...
    // If no lobbies still have "availability", just fill any lobby. CHANGED
    if (lobbyPool.length === 0) {
	return;
    }

    // Sort the lobbies by the number of players in them
    lobbyPool.sort((a, b) => a.availableCount < b.availableCount);

    // Pick the first lobby with which there is no collision  
    const lobby = greedyNoMatches(lobbyPool, player.id);
    if (!lobby) {
      return;
    }
